...
parameters = [r"$A_{K'}$", r"$R_{1}$ $(R_{\odot})$", r"$R_{2}$ $(R_{\odot})$", r"$i$ $(\degree)$", r"$P$ (d)", r"$d$ (pc)", r"$t_0$ (MJD)"]
...
samples = np.loadtxt('./chains_try{0}.dat'.format(trial_num))

walker_indices = np.unique(samples[:, 0])

burn_ignore_len = 0
burn_ignore_len = 500
last_iterations_count = -1
num_walkers = len(walker_indices)

## Organize into shape that can work for us
if last_iterations_count == -1:
    samples = samples[burn_ignore_len * num_walkers:, 1:]
else:
    samples = samples[(-1 * last_iterations_count * num_walkers):, 1:]
# print(samples)
# samples = np.swapaxes(samples, 0, 1)


samples_Kp_ext = samples[:, 0]
samples_star1_rad = samples[:, 1]
samples_star2_rad = samples[:, 2]
samples_bin_inc = samples[:, 3]
samples_binary_period = samples[:, 4]
samples_binary_dist = samples[:, 5]
samples_t0 = samples[:, 6]

## Print out MCMC results
### Compute median centered 1 sigma regions
Kp_ext_mcmc, star1_rad_mcmc, star2_rad_mcmc, bin_inc_mcmc, binary_period_mcmc, binary_dist_mcmc, t0_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), zip(*np.percentile(samples, [15.866, 50., 84.134], axis=0)))
