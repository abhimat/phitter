...
def single_star_lc(stellar_params,
    ...
    (star_mass, star_rad, star_teff,
     star_mag_Ks, star_mag_H, star_pblum_Ks, star_pblum_H) = stellar_params
    ...
    if use_blackbody_atm:
        sing_star.add_dataset(phoebe.dataset.lc, times=[0], dataset='mod_lc_Ks', passband='2MASS:Ks', ld_func='linear', ld_coeffs=[0.0])
        sing_star.add_dataset(phoebe.dataset.lc, times=[0], dataset='mod_lc_H', passband='2MASS:H', ld_func='linear', ld_coeffs=[0.0])
        # sing_star.add_dataset(phoebe.dataset.lc, times=[0], dataset='mod_lc_V', passband='Johnson:V', ld_func='linear', ld_coeffs=[0.0])
    else:
        sing_star.add_dataset(phoebe.dataset.lc, times=[0], dataset='mod_lc_Ks', passband='2MASS:Ks')
        sing_star.add_dataset(phoebe.dataset.lc, times=[0], dataset='mod_lc_H', passband='2MASS:H')
    ...
    sing_star.set_value('pblum@mod_lc_Ks', star_pblum_Ks)
    ...
    sing_star_fluxes_Ks = np.array(sing_star['fluxes@lc@mod_lc_Ks@model'].value) * u.W / (u.m**2)
    sing_star_mags_Ks = -2.5 * np.log10(sing_star_fluxes_Ks / flux_ref_Ks) + 0.03
    ...
    return (sing_star_mags_Ks, sing_star_mags_H)
...
def binary_star_lc(star1_params, star2_params, binary_params, observation_times,
    ...
    (star1_mass, star1_rad, star1_teff,
     star1_mag_Ks, star1_mag_H,
     star1_pblum_Ks, star1_pblum_H) = star1_params
    (star2_mass, star2_rad, star2_teff,
     star2_mag_Ks, star2_mag_H,
     star2_pblum_Ks, star2_pblum_H) = star2_params
    ...
    ## Phase the observation times
    ks_phased_days = ((ks_MJDs - t0) % binary_period.to(u.d).value) / binary_period.to(u.d).value
    h_phased_days = ((h_MJDs - t0) % binary_period.to(u.d).value) / binary_period.to(u.d).value
    
    # Add light curve datasets
    ## Ks
    ks_phases_sorted_inds = np.argsort(ks_phased_days)
    
    ks_model_times = (ks_phased_days) * binary_period.to(u.d).value
    ks_model_times = ks_model_times[ks_phases_sorted_inds]
    
    if use_blackbody_atm:
        b.add_dataset(phoebe.dataset.lc, time=ks_model_times, dataset='mod_lc_Ks', passband='2MASS:Ks', ld_func='linear', ld_coeffs=[0.0])
    else:
        b.add_dataset(phoebe.dataset.lc, time=ks_model_times, dataset='mod_lc_Ks', passband='2MASS:Ks')
    ...
    if use_blackbody_atm:
        b.add_dataset(phoebe.dataset.lc, times=h_model_times, dataset='mod_lc_H', passband='2MASS:H', ld_func='linear', ld_coeffs=[0.0])
    else:
        b.add_dataset(phoebe.dataset.lc, times=h_model_times, dataset='mod_lc_H', passband='2MASS:H')
    ...
    b.set_value('pblum@primary@mod_lc_Ks', star1_pblum_Ks)
    b.set_value('pblum@primary@mod_lc_H', star1_pblum_H)
    
    b.set_value('pblum@secondary@mod_lc_Ks', star2_pblum_Ks)
    ...
    # Get fluxes
    ## Ks
    model_fluxes_Ks = np.array(b['fluxes@lc@mod_lc_Ks@model'].value) * u.W / (u.m**2.)
    model_mags_Ks = -2.5 * np.log10(model_fluxes_Ks / flux_ref_Ks) + 0.03
    ...
    if print_diagnostics:
        print('\nFlux Checks')
        print('Fluxes, Ks: {0}'.format(model_fluxes_Ks))
        print('Mags, Ks: {0}'.format(model_mags_Ks))
        print('Fluxes, H: {0}'.format(model_fluxes_H))
        print('Mags, H: {0}'.format(model_mags_H))
    
    return (model_mags_Ks, model_mags_H)
...
def phased_obs(observation_times, binary_period, t0):
    ...
    return ((ks_phased_days, ks_phases_sorted_inds, ks_model_times),
            (h_phased_days, h_phases_sorted_inds, h_model_times))


def dist_ext_mag_calc(input_mags, target_dist, Ks_ext, H_ext):
    (mags_Ks, mags_H) = input_mags
    
    # App mag at target distance (default system dist = 10 pc)
    mags_Ks = mags_Ks + 5. * np.log10(target_dist / (10. * u.pc))
    mags_H = mags_H + 5. * np.log10(target_dist / (10. * u.pc))
    
    # Add extinction
    mags_Ks = mags_Ks + Ks_ext
    mags_H = mags_H + H_ext
    
    # Return mags at target distance and extinction
    return (mags_Ks, mags_H)
...
def flux_adj(mags_pri, mags_ref_pri, mags_sec, mags_ref_sec, mags_bin):
    ...
    * Applies correction to the calculated binary magnitudes
    ...
    (mag_pri_Ks, mag_pri_H) = mags_pri
    (mag_ref_pri_Ks, mag_ref_pri_H) = mags_ref_pri
    
    (mag_sec_Ks, mag_sec_H) = mags_sec
    (mag_ref_sec_Ks, mag_ref_sec_H) = mags_ref_sec
    
    (mags_bin_Ks, mags_bin_H) = mags_bin
    
    # Calculate flux adjustment for each component star
    flux_adj_star_pri_Ks = (10.**((mag_pri_Ks[0] - mag_ref_pri_Ks)/2.5))
    flux_adj_star_pri_H = (10.**((mag_pri_H[0] - mag_ref_pri_H)/2.5))
    
    flux_adj_star_sec_Ks = (10.**((mag_sec_Ks[0] - mag_ref_sec_Ks)/2.5))
    flux_adj_star_sec_H = (10.**((mag_sec_H[0] - mag_ref_sec_H)/2.5))
    
    # Calculate total flux adjustment for the binary system
    flux_adj_bin_Ks = ((10.**(mag_ref_pri_Ks / -2.5)) + (10.**(mag_ref_sec_Ks / -2.5))) / ((10.**(mag_pri_Ks[0] / -2.5)) + (10.**(mag_sec_Ks[0] / -2.5)))
...
def binary_mags_calc(star1_params_lcfit, star2_params_lcfit,
    ...
    (star1_mass, star1_rad, star1_teff,
     star1_mag_Ks, star1_mag_H, star1_pblum_Ks, star1_pblum_H) = star1_params_lcfit
    (star2_mass, star2_rad, star2_teff,
     star2_mag_Ks, star2_mag_H, star2_pblum_Ks, star2_pblum_H) = star2_params_lcfit
    
    
    # Run binary star model to get binary mags
    (binary_mags_Ks, binary_mags_H) = binary_star_lc(
                                          star1_params_lcfit,
                                          star2_params_lcfit,
                                          binary_params,
                                          observation_times,
